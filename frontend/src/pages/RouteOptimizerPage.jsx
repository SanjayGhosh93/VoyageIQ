          ...
          {['Lowest Cost', 'Lowest Risk', 'Fastest Delivery', 'Balanced'].map((p) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-mono font-semibold transition-all ${
                priority === p
                  ? 'bg-gradient-to-r from-ocean-500 to-cyan-500 text-slate-950 font-bold shadow-md'
        ...
        <div className="p-6 rounded-3xl glass-panel space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            ...
            <span className="text-xs font-mono text-slate-400">
              Optimal: <strong className="text-amber-400">{routesData?.bestRoute?.destination} Port</strong>
            </span>
          </div>
            ...
            {routesData?.routes?.map((r, idx) => (
              <div
                ...
                className={`p-5 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between ${
                  selectedRoute?.destination === r.destination
                    ? 'bg-gradient-to-b from-slate-900 to-navy-950 border-cyan-400/60 shadow-xl shadow-cyan-500/10 ring-1 ring-cyan-400/30'
              ...
              >
                <div>
                  ...
                  <h4 className="text-base font-bold text-white mb-1">{r.routeName}</h4>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs font-mono mb-3">
                    <div className="p-2 rounded-xl bg-slate-900 border border-slate-800">
                      <span className="text-slate-500 block text-[9px] uppercase">Landed / MT</span>
                      <span className="text-cyan-400 font-bold">{formatCurrencyPerMT(r.costPerMT)}</span>
                    </div>
                    <div className="p-2 rounded-xl bg-slate-900 border border-slate-800">
                      <span className="text-slate-500 block text-[9px] uppercase">Demurrage Risk</span>
                      <span className={`font-bold ${r.riskScore > 50 ? 'text-amber-400' : 'text-emerald-400'}`}>
                        {r.riskScore}/100
                      </span>
                    </div>
                  </div>
                ...
                <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs font-mono">
                  <span className="text-slate-400">Score: <strong className="text-white font-bold">{r.optimizationScore}/100</strong></span>
                  <span className="text-ocean-400 font-semibold flex items-center gap-1">
                    <span>Inspect</span>
