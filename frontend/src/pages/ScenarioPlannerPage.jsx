    ...
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-3xl bg-gradient-to-r from-navy-900 via-slate-900 to-navy-950 border border-ocean-500/25 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-gradient-to-r from-ocean-500 to-cyan-500 hover:from-ocean-400 hover:to-cyan-400 text-slate-950 font-extrabold text-xs font-mono transition-all shadow-md shadow-ocean-500/20 flex items-center gap-1.5"
        >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs font-mono">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {scenarios.map((s, idx) => (
                    <tr key={s._id || idx} className="hover:bg-slate-900/60 transition-colors">
                      <td className="py-3 pr-2 font-sans font-semibold text-slate-200">
                        <div className="flex items-center gap-2">
                      ...
                      <td className="py-3 text-slate-300 font-semibold">{s.vesselClass}</td>
                      <td className="py-3 text-slate-400">{s.originPort} → {s.destinationPort}</td>
                      <td className="py-3 text-right text-slate-300">${s.freightRateUSDPerMT?.toFixed(2)}</td>
                      <td className="py-3 text-right text-rose-400">{formatCurrency(s.demurrageExposureUSD)}</td>
                      <td className="py-3 text-right">
                        <span className={`font-bold ${s.riskScore > 50 ? 'text-amber-400' : 'text-emerald-400'}`}>
                          {s.riskScore}/100
                        </span>
                      </td>
                      <td className="py-3 text-right text-slate-200">{formatCurrency(s.totalLandedCostUSD)}</td>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="w-full max-w-xl p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-white">Create New Charter Scenario</h3>
            <form onSubmit={handleCreate} className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">Scenario Title</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Cargo Parcel (MT)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Cargo Type</label>
                  <select
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Origin Port</label>
                  <select
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Destination Port</label>
                  <select
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Vessel Class</label>
                  <select
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Contract Type</label>
                  <select
                    value={newScenario.contractType}
                    onChange={(e) => setNewScenario({ ...newScenario, contractType: e.target.value })}
                    className="w-full glass-input rounded-xl px-3 py-2 text-xs text-white"
                  >
                    <option value="Spot Voyage" className="bg-slate-900">Spot Voyage</option>
                    <option value="Time Charter" className="bg-slate-900">Time Charter</option>
                    <option value="COA" className="bg-slate-900">COA</option>
                  </select>
                </div>
              </div>
              ...
              <div className="pt-3 flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-ocean-600 hover:bg-ocean-500 text-white font-bold text-xs font-mono"
                >
