    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-mono font-semibold transition-all ${
              regionFilter === 'ALL' ? 'bg-ocean-500 text-white font-bold' : 'bg-slate-900 text-slate-400 border border-slate-800'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-mono font-semibold transition-all ${
              regionFilter === 'East Coast India' ? 'bg-ocean-500 text-white font-bold' : 'bg-slate-900 text-slate-400 border border-slate-800'
            }`}
          >
            East Coast India (9)
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-mono font-semibold transition-all ${
              regionFilter === 'Overseas' ? 'bg-ocean-500 text-white font-bold' : 'bg-slate-900 text-slate-400 border border-slate-800'
            }`}
          >
            Overseas Origins (8)
          </button>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredPorts.map((p) => (
            <div key={p.name} className="p-5 rounded-3xl glass-panel flex flex-col justify-between space-y-4">
              <div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-slate-800 border border-slate-700 text-cyan-300">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs font-mono mt-3">
                  <div className="p-2 rounded-xl bg-slate-900/80 border border-slate-800">
                    ...
                    <span className="text-white font-bold">{p.berthDraft}m</span>
                  </div>
                  <div className="p-2 rounded-xl bg-slate-900/80 border border-slate-800">
                    <span className="text-slate-500 block text-[9px] uppercase">Max LOA / Beam</span>
                    <span className="text-white font-bold">{p.maxLOA}m / {p.maxBeam}m</span>
                  </div>
                  <div className="p-2 rounded-xl bg-slate-900/80 border border-slate-800">
                    <span className="text-slate-500 block text-[9px] uppercase">Discharge TPH</span>
                    <span className="text-cyan-400 font-bold">{p.handlingRateTPH} TPH</span>
                  </div>
                  <div className="p-2 rounded-xl bg-slate-900/80 border border-slate-800">
                    <span className="text-slate-500 block text-[9px] uppercase">Avg Waiting</span>
                    <span className="text-amber-400 font-bold">{p.averageWaitingDays || 2.0} Days</span>
                  </div>
                </div>
